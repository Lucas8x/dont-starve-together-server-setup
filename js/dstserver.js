$(document).ready(function () {
  ...
  getFromClusterSettingsObjectList = id => {
    return cluster_settings_object_list.filter(setting => setting.game_id === id)[0];
  }
  ...
  generateClusterIni = () => {
    let cluster_ini = (`[GAMEPLAY]\ngame_mode = ${getFromClusterSettingsObjectList('game_mode').getActualValue()}\nmax_players = ${$("input[name=max_players]").val()}\npvp = ${getFromClusterSettingsObjectList('pvp').getActualValue()}\npause_when_empty = ${getFromClusterSettingsObjectList('pause_when_empty').getActualValue()}\n[NETWORK]\ncluster_name = ${$("input[name=cluster_name]").val()}\ncluster_description = ${$("input[name=cluster_description]").val()}\ncluster_password = ${$("input[name=password]").val()}\ncluster_intention = ${cluster_intention.toLowerCase()}\nlan_only_cluster = ${getFromClusterSettingsObjectList('lan_only_cluster').getActualValue()}\noffline_cluster = ${getFromClusterSettingsObjectList('offline_cluster').getActualValue()}\ncluster_language = ${getFromClusterSettingsObjectList('cluster_language').getActualValue()}\nwhitelist_slots = ${$("input[name=whitelist_slots]").val()}\nautosaver_enabled = ${getFromClusterSettingsObjectList('autosaver_enabled').getActualValue()}\nenable_vote_kick = ${getFromClusterSettingsObjectList('enable_vote_kick').getActualValue()}\ntick_rate = ${$("input[name=tick_rate]").val()}\n[MISC]\nconsole_enabled = ${getFromClusterSettingsObjectList('console_enabled').getActualValue()}\nmax_snapshots = ${$("input[name=max_snapshots]").val()}\n[STEAM]\nsteam_group_only = ${checkSteamGroup()}\nsteam_group_id = ${$("input[name=steam_group_id]").val()}\nsteam_group_admins = ${$("span[name=steam_group_admins]").text().toLowerCase()}\n[SHARD]\nshard_enabled = ${checkIfCavesEnabled()}\nbind_ip = 127.0.0.1\nmaster_ip = 127.0.0.1\nmaster_port = 10888\ncluster_key = defaultPass`);
    return cluster_ini;
  }
